#!/usr/bin/env python
## setupVisit.py - bookkeeping for initiating a phoSim run in DC1-phoSim task
##
...
sys.path.insert(1,os.getenv('DC1_CONFIGDIR'))
#from config import *
from utils import *

## Say hello
sstream = os.getenv('DC1_SIXDIGSTREAM')
log.info("Starting stream %s",sstream)
## Preserve 6-digit top-level stream number
cmd = 'pipelineSet DC1_TOPLEVEL6 '+sstream
rc = os.system(cmd)
if rc <> 0 :
...
## Directory structure: $DC1_ROOT/<output>/<stream>/{work,output}
filePermissions = int(os.getenv("filePermissions"),8) ## Octal file permissions
archivesDirName = 'archives'
outDir = os.path.join(os.environ['DC1_OUTPUT'],os.environ['DC1_SIXDIGSTREAM'])
...
cmd = 'pipelineSet DC1_PHOSIMOUT '+outDir
print cmd
rc = os.system(cmd)
if rc <> 0 :
    log.error("Unable to set pipeline variable \n $%s",cmd)
...
print 'foo = ',foo
print 'bar = ',bar
cmd = 'pipelineSet DC1_SENSORLIST_1 '+foo
rc1 = os.system(cmd)
cmd = 'pipelineSet DC1_SENSORLIST_2 '+bar
rc2 = os.system(cmd)
cmd = 'pipelineSet DC1_NUM_SENSORS '+str(len(sensorList))
rc3 = os.system(cmd)
...
cmd = 'pipelineSet DC1_INSTANCE_CATALOG '+icSelect
rc = os.system(cmd)
if rc <> 0 :
    log.error("Unable to set pipeline variable")
    sys.exit(99)
...
cmd = 'pipelineSet DC1_NSNAP '+nsnap
print cmd
rc = os.system(cmd)
if rc <> 0 :
    log.error("Unable to set pipeline variable \n $%s",cmd)
    sys.exit(99)
    pass
cmd = 'pipelineSet DC1_OBSHISTID '+visit
print cmd
rc = os.system(cmd)
if rc <> 0 :
    log.error("Unable to set pipeline variable \n $%s",cmd)
    sys.exit(99)
    pass
cmd = 'pipelineSet DC1_FILTER '+cfilter
print cmd
rc = os.system(cmd)
if rc <> 0 :
    log.error("Unable to set pipeline variable \n $%s",cmd)
...
cmd = 'pipelineSet DC1_CENTROIDFILE '+centroidfile
rc = os.system(cmd)
if rc <> 0 :
    log.error("Unable to set pipeline variable \n $%s",cmd)
    sys.exit(99)
...
cmd = 'pipelineSet DC1_NTRIMS '+str(ntrim)
rc = os.system(cmd)
if rc <> 0 :
    log.error("Unable to set pipeline variable \n $%s",cmd)
    sys.exit(99)
